...
plusPlus :: Lst -> Lst -> Lst
rev :: Lst -> Lst
plusPlus Nil r = r
plusPlus (Cons a l) r = (Cons a (plusPlus l r))
rev Nil = Nil
rev (Cons x xs) = (plusPlus (rev xs) (Cons x Nil))
prop x = proveBool $  (eq (plusPlus x (plusPlus (rev x) x)) (plusPlus (plusPlus x (rev x)) x))
